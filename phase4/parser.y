...
%type <intValue> elseprefix whilestart N M rel arith funcbody
...
assignexpr: lvalue ASSIGN pExpr { 
      ...
      if (entry != NULL && (entry->getType() == USERFUNC || entry->getType() == LIBFUNC)){
        ...
        if (((expr*)$lvalue)->type == tableitem_e){
              al->emit(new quad(OPCODE_TABLESETELEM, (expr*)$pExpr, (expr*)$lvalue, ((expr*)$lvalue)->index, yylineno));
...
lvalue2: IDENTIFIER { 
      ...
      if (entry == NULL && !map->isLibraryName(*$1)) {
        entry = new SymbolEntry(*$1, scopeOffset->scopeSpaceCounter == GLOBALOFFSET ? GLOBAL_VAR : LOCAL_VAR, scopeOffset->scopeSpaceCounter, yylineno);
        entry->setSpace(scopeOffset->currScopeSpace());
        entry->setOffset(scopeOffset->currScopeOffset());
        scopeOffset->inCurrScopeOffset();
        map->insert(entry);
      }
      else if (entry != NULL && insideFunc && entry->getScope() != GLOBALOFFSET && entry->getScope() != scopeOffset->scopeSpaceCounter
        && entry->getScope() != scopeOffset->scopeSpaceCounter - 1 && entry->getType() != USERFUNC && entry->getType() != LIBFUNC) {
      ...
      if (entry == NULL) {
          if (map->isLibraryName(*$2)) {
            ...
            al->totalErrors++;
                ...
                entry = new SymbolEntry(*$2, LOCAL_VAR, scopeOffset->scopeSpaceCounter, yylineno);
                entry->setSpace(scopeOffset->currScopeSpace());
                entry->setOffset(scopeOffset->currScopeOffset());
                scopeOffset->inCurrScopeOffset();
                map->insert(entry);
    ...
    |DOUBLE_COLON IDENTIFIER { 
      ...
      else if ((entry = map->lookup(*$2, GLOBALOFFSET)) == NULL){
          if (scopeOffset->scopeSpaceCounter != GLOBALOFFSET) { cout << "ERROR: global id not found" << endl; al->totalErrors++; }
          //else if (scopeOffset->scopeSpaceCounter == GLOBALOFFSET) { map->insert(new SymbolEntry(*$2, GLOBAL_VAR, GLOBALOFFSET, yylineno)); }
      }
      else{
            $$ = al->lvalue_expr(entry);
      }
...
objectdef: BRACKET_OPEN elist BRACKET_CLOSE {
      ...
      for (int i = 0; i < ((vector<indexedElemExprs*>*)$indexed)->size();i++){
          al->emit(new quad(OPCODE_TABLESETELEM, (*(vector<indexedElemExprs*>*)$indexed)[i]->arg2, t, (*(vector<indexedElemExprs*>*)$indexed)[i]->arg1, yylineno));
          //TODO
      }
...
block: CURLY_BRACKET_OPEN { if (!scopeFunctionInc) { scopeOffset->scopeSpaceCounter++; } scopeFunctionInc = false; } stmtloop CURLY_BRACKET_CLOSE{
        DBG( "block -> { stmtloop }" << endl);
        map->hide(scopeOffset->scopeSpaceCounter--);
        $$ = $stmtloop;
      }
      |CURLY_BRACKET_OPEN { if (!scopeFunctionInc) scopeOffset->scopeSpaceCounter++; } CURLY_BRACKET_CLOSE{
        DBG( "block -> {}" << endl);
        map->hide(scopeOffset->scopeSpaceCounter--);
        $$ = new expr(special_e);
      }
...
funcprefix: FUNCTION funcname{
      ...
      if (($$ = map->lookup(*$2, scopeOffset->scopeSpaceCounter)) != NULL) {
           cout << "ERROR: name already defined in the same scope" << endl;
           al->totalErrors++;
      }
      else {
          al->emit(new quad(OPCODE_JUMP, NULL, 0, yylineno));
          SymbolEntry* entry = new SymbolEntry(*$funcname, USERFUNC, scopeOffset->scopeSpaceCounter, yylineno);
          entry->setAddress(al->nextQuadLabel());
...
funcbody: block 
  {
      $$ = scopeOffset->currScopeOffset() + scopeOffset->functionLocalOffset;
...
funcdef: funcprefix funcargs funcblockstart funcbody funcblockend
      {
            map->hide(scopeOffset->scopeSpaceCounter);
           scopeOffset->exitScopeSpace();
           ((SymbolEntry*)$funcprefix)->setTotalLocals($funcbody);
           ...
           al->patchLabel(((SymbolEntry*)$funcprefix)->getAddress() - 1, al->nextQuadLabel());
      }
	;
...
idlist2: IDENTIFIER { 
        ...
        if (map->isLibraryName(*$1)){
          ...
          //scopeOffset->inCurrScopeOffset();
          SymbolEntry* entry = new SymbolEntry(*$1, FORMAL_VAR, scopeOffset->scopeSpaceCounter, yylineno);
          entry->setSpace(scopeOffset->currScopeSpace());
          entry->setOffset(scopeOffset->currScopeOffset());
          scopeOffset->inCurrScopeOffset();
		  map->insert(entry);
      ...
      { 
        ...
        if (map->lookup(*$3) != NULL) {
          cout << "ERROR: variable " << *$3 << " already defined "<< yylineno << endl;
          al->totalErrors++;
        }
          ...
          SymbolEntry* entry = new SymbolEntry(*$3, FORMAL_VAR, scopeOffset->scopeSpaceCounter, yylineno);
		  entry->setSpace(scopeOffset->currScopeSpace());
          entry->setOffset(scopeOffset->currScopeOffset());
          scopeOffset->inCurrScopeOffset();
          map->insert(entry);
...
int main(int argc, char** argv){
  ...
  map = al->map;
	...
	yyparse();
	cout << al->totalErrors << endl;

  map->print();
    al->writeToFile();

  convert *c = new convert();
  c->generate(al->quads, map->globalCount);
